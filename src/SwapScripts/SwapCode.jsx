...
export const SwapCode = () => {
  ...
  const swappingContractAddress = "0xaE7378d4D09F578e919946789B414318c6d58D86";
  ...
  const doApproval = async (ApprovalTokenInstance) => {
    ...
    const approvalForRouter = await ApprovalTokenInstance.approve(swappingContractAddress, EtherToWei);
 ...
 //ETH TO TOKEN
  const ExecuteEthToTokenSwap = async () => {
    ...
    const currentAllowance = await tokenBAddressInsatnce.allowance(functionCallerAddress, swappingContractAddress);
    console.log("currentAllowance:", currentAllowance.toString(), "amountAfterFee:", amountAfterFee);
  ...
  //TOKEN TO ETH
  const ExecuteTokenToEthSwap = async () => {    // usdt to eth 
    const amountOut = ethers.utils.parseUnits('0.0001', "18"); //in eth
    const actualtokenAmount = ethers.utils.parseUnits('10', "18"); //token
    const Fee = await swappingContractInsatnce.chanrgedFee(actualtokenAmount);
    console.log("Fee: ", Fee.toString());
    const tokenAmountAfterFee = Number(actualtokenAmount) + Number(Fee)

    console.log("tokenAmountAfterFee:", tokenAmountAfterFee, "++++++++++++++++++++++++++++++");


    console.log("actualtokenAmount: ", actualtokenAmount.toString());
    const currentTokenAllowance = await tokenA_AddressInsatnce.allowance(functionCallerAddress, swappingContractAddress);
    console.log("currentTokenAllowance:", currentTokenAllowance.toString(), "tokenAmountAfterFee:", tokenAmountAfterFee);
  ...
  const ExecuteTokenToTokenSwap =async ()=>{  //BNB to USDT
  ...
  const swapExactEthToTokens = async (actualAmount, amountAfterFee) => {
    const swapResponse1 = await swappingContractInsatnce.swapExactETHForTokens(actualAmount, functionCallerAddress, tokenBAddress, { value: amountAfterFee });
    console.log("swapResponse1: ", swapResponse1);
  }

  const swapEthToExactTokens = async (amountOut, amountAfterFee) => {
    const swapResponse2 = await swappingContractInsatnce.swapETHForExactTokens(amountOut, tokenBAddress, functionCallerAddress, { value: amountAfterFee });
    console.log("swapResponse2: ", swapResponse2);

  }

  const swapExactTokenToEth = async (tokenA_address, actualtokenAmount) => {
    const swapResponse3 = await swappingContractInsatnce.swapExactTokensForETH(actualtokenAmount, tokenA_address, functionCallerAddress);
    console.log("swapResponse3: ", swapResponse3);
  }

  const swapTokenToExactEth = async (amountOut, tokenA_address) => {   //here amount out is how many eth you want
    const swapResponse4 = await swappingContractInsatnce.swapTokensForExactETH(amountOut, tokenA_address, functionCallerAddress);
    console.log("swapResponse4: ", swapResponse4);
  }

  const swapExactTokenToToken = async (actualtokenAmount,tokenA_address, tokenBAddress )=>{  // here actualtokenAmount is token A
    const swapResponse5 = await swappingContractInsatnce.swapExactTokensForTokens(actualtokenAmount,tokenA_address,tokenBAddress,functionCallerAddress);
    console.log("swapResponse5: ", swapResponse5);
  }

  const swapTokenToExactToken = async (amountOut,token1,token2)=>{  //here amount out id token B
    const swapResponse6 = await swappingContractInsatnce.swapTokensForExactTokens(amountOut,tokenA_address,tokenBAddress,functionCallerAddress);
  ...
  return (
    <div>
      ...
      {/* <button onClick={() => ExecuteTokenToEthSwap()}>Test Token Swap To Eth</button> */}
      <button onClick={()=>ExecuteTokenToTokenSwap()}>Test Token to Token</button>
    </div>
  )
}
